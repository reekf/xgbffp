# Actual PyFLEXTRKR HRRR MCS classification

Realtime publication and the 2026 archive audit use an HRRR classification
from the upstream PyFLEXTRKR package, not an independent lifecycle
approximation. The adapter invokes these official package stages:
...
HRRR uses its 12Z f00-f24 forecast to evaluate the 12Z-to-12Z valid window.
Only HRRR must qualify for a forecast to be MCS-eligible. RAP is not used by
the operational MCS gate.
...
input manifest, and result JSON are stored below each HRRR trigger cache case.
...
closed; there is no fallback custom tracker. HRRR always requires SBT.
